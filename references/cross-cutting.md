...
**Integration:**
- `prep` and persona practice include a brief psychological warmup and mid-interview recovery scripts when anxiety is the active bottleneck.
...
| `decode` | Profile from `kickoff`, Resume Analysis, Storybank | Profile (can decode JD language but cannot assess fit), Resume Analysis / Storybank (cannot map competencies to candidate) | JD text (at least one) |
