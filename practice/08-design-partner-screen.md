...
>
> At NASA Langley I worked on medical workstations for Lunar Gateway, where I watched procedures, catalogued the use errors, and redesigned the displays and layout around them. Task time came down thirty percent and the critical input errors went away.
>
...
> I'd spend it watching before touching. I'd sit with the flow end to end and find where the interface is competing with the mission instead of serving it. Then I'd do one thing fast — an edge-case and error list for whichever surface is closest to shipping, in your format rather than mine. I'd want that month judged on whether it changed a screen, not on whether it was thorough.
...
> My default is fast, and then I earn the right to slow down. I'd rather start with observation this week and discover the question is bigger than I thought than spend three weeks designing a study for a problem that turns out to be a copy change.
...
> That would be the Uber work, and it was scrappy because it had to be. The question was why drivers were struggling with navigation in dense urban cores, and there was no version of that answerable from a desk.
>
> So I went out with them. I did ride-alongs on real shifts and watched what they did at the moments when the interface and the street were competing for the same attention. It was less a protocol than a hunt for the tells — a glance held too long, a reroute they ignored, a pull-over to re-read the screen.
>
> What came out of it was a small set of failures that only exist in the environment. There was glare, there was one-handed use, and there were decision points that arrived after the turn was already committed. None of that surfaces in a lab, where the participant has both hands free and no traffic.
>
> It went back to design as an edge-case list they iterated on across releases, and it was part of a five percent increase in driver retention.

**F1 — "How many drivers?"** *(~55 words)*

> It was a handful, which is the right answer for that question. I wasn't estimating a rate, I was finding failure modes, and failure modes saturate fast — by the third or fourth ride-along I was seeing the same three things. If I'd needed to know how *often* it happened, that's a survey, not a ride-along.
...
> The clearest case is the response-time work at Amazon. Engineering had targets that were essentially invented — round numbers with no human behind them — and everyone was arguing from intuition.
>
...
> I'd take the one interaction that's actually in dispute, run six to eight people against the real build, and vary one thing. You'd have a defensible answer in two days. What you would not have is anything that generalizes past that interaction, and I'd say so in the first line of the readout rather than the last.
...
>
> The objective piece was fNIRS — near-infrared light through the forehead, reading how hard the front of the brain is working. It's less exotic than it sounds. In practice it's a headband, a clean baseline, and the same person doing the same task across two interfaces. What you get out of it is mental effort the person can't self-report, measured continuously while they work.
>
...
> Then I'd fall back on the cheap proxies, and they're decent. I'd look at time to first correct fixation, how often someone backtracks, error rate on a timed task, and a complexity rating taken right afterward. That gives you a direction in a day or two. What it won't tell you is *why* one layout costs more, which is where the instrumented version earns its keep.
...
> There are three things, and none of them is a deck that closes out a project.
>
> The first is a short insight report — what I saw, what I think it means, and how confident I am line by line. I want you reading my uncertainty, not just my conclusions.
>
> The second is a blueprint of the operator's flow with the failure points marked, showing where what the person expects and what the system is doing come apart. That becomes the shared object between us, so when we disagree later we're disagreeing about the model rather than about taste.
>
> The third is an annotated list of edge cases, which are the states nobody thinks about. A degraded feed, contradictory tracks, an alert landing mid-input, someone coming back after ninety seconds away.
>
> All three are insight for you to iterate against, not requirements I'm handing down. You decide what the interface does about them, and I'd rather be in the room while you do that than send a link afterward.

**F1 — "How fast?"** *(~50 words)*

> You'd get notes the same day and the tidy version within two. If a readout takes me a week to write, the sprint has already moved past it and I've produced a historical document. I'd rather give you something rough on Tuesday that's still in time to matter.
...
> Yes, as long as the risk is named. Most design decisions are reversible, and blocking those on evidence costs more than being wrong. The irreversible ones are where I'd hold the line, and in any given flow there are two or three of those, not thirty.
...
> This was the same cognitive-load program at Amazon. The eye tracking showed a region of the interface that people simply never looked at, and my read was salience — make it louder, move it up.
>
...
> The clearest one is the frictionless-experience argument, and I lost the first round of it.
>
...
>
> In practice that's two error rates rather than one — the times someone accepted a recommendation that was wrong, and the times they overrode one that was right. Those are different failures with different design fixes, and a single accuracy number hides both. Then I'd look at whether the interface's expression of uncertainty actually moves behavior, because if confidence is displayed and reliance doesn't change with it, the display is decoration.
>
...
>
> That makes the accessibility toolkit the right toolkit rather than an adjacent one. You want redundant coding, because color alone fails at night. You want confirmation of what the system believes it received, because an unacknowledged input under load gets repeated. You want targets sized for gloved hands, and recovery that costs less than the error.
>
...
> Yes, on automotive handovers. I split takeover time into hands on the wheel, eyes on the road, and first control input, because one reaction time hides which stage failed. Multimodal alerts came out 24% better on safety and trust than the single-channel versions.
...
>
> What I build instead is a task model. It lays out the decision sequence, what the person needs to know at each step, what they're holding in their head, and what the interface is competing with for attention. Then the parts that can carry numbers get them.
>
...
> It's weeks to be useful and months to be right. The accelerant is watching people work, because an hour of that beats a week of documents. I'd expect to be corrected early and often, and I'd rather be corrected than careful, because careful is slow and slow is the real risk.
...
> My rule is to cut scope and never cut controls. One interaction with the confounds handled is useful. Six interactions with the confounds loose isn't a smaller finding, it's a wrong one — and a wrong number travels faster than no number, because it's quotable.
>
...
> I'd say five structured observations against a build, timed, with the errors classified. That's a day of work and it will change a design. What it won't do is establish a rate — and knowing which of those two I'm being asked for is most of the job.
...
> It's more hands-on than the title implies, and I want it that way.
>
...
> Honestly, there's nothing on that list I'd avoid. What I'd push back on isn't a task, it's a pattern — being brought in after the decision to validate it. Give me the ugly part of the flow early and I'll take whatever method it needs, including sitting there with a stopwatch.
...
> Where would research actually plug into your week, and what would you want in your hands? A raw edge-case list mid-sprint and a synthesized model at the start of a cycle are different jobs, and I'd rather do the one you need.

**RQ4 — Degraded conditions, codified or re-argued.**
> How much of the degraded-conditions envelope is codified as a design constraint versus re-argued screen by screen? I'm thinking about gloved input, glare, noise, and sustained wear.
