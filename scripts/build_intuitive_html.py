...
DOCS = {
    ...
    "onsite": {
        "src": ROOT / "intuitive" / "onsite-guide.md",
    ...
    "checklist": {
        "src": ROOT / "intuitive" / "onsite-checklist.md",
        "out": OUT / "onsite-checklist.html",
        "title": "Before the onsite — checklist",
        "break_before_part1": False,
        "body_class": "compact",
    },
    "anduril": {
...
@media print {
  ...
  /* 25 answers against a 20-page cap: a hair tighter than the long guide. */
  body.compact { font-size: 9.6pt; line-height: 1.31; }
...
def wrap_question_heads(body: str) -> str:
    ...
    pattern = re.compile(
        r"(<h2>(?:(?:EQ|MQ|RQ|SQ)\d+|[^<]*\u2014 Tier \d)[^<]*</h2>\s*)"
        r"(<p class=\"meta\">.*?</p>\s*)?"
...
def wrap_model_answers(
    ...
    pattern = re.compile(
        r"(<h3>(?:Model answer[^<]*|Answer)</h3>\s*)((?:<p>(?:(?!</?h[1-4]|<blockquote|<hr).)*?</p>\s*)+)",
        flags=re.DOTALL,
    )
    body = pattern.sub(
